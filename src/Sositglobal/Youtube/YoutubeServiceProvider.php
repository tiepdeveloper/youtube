...
class YoutubeServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        if ($this->isLegacyLaravel() || $this->isOldLaravel()) {
            $this->package('Sositglobal/youtube', 'Sositglobal/youtube');
        }

        $loader = \Illuminate\Foundation\AliasLoader::getInstance();
        $loader->alias('Youtube', 'Sositglobal\Youtube\Facades\Youtube');
    ...
    public function register()
    {
        if ($this->isLegacyLaravel() || $this->isOldLaravel()) {
            $this->app['youtube'] = $this->app->share(function ($app) {
                $key = \Config::get('Sositglobal/youtube::KEY');
        ...
        if (floatval(Application::VERSION) >= 5.4) {
            ...
            $this->app->bind("youtube", function(){
                return $this->app->make('Sositglobal\Youtube\Youtube', [config('youtube.KEY')]);
            });
        }else{
